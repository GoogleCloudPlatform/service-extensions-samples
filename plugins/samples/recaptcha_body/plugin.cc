...
class MyRootContext : public RootContext {
 public:
  ...
  bool onConfigure(size_t) override {
    ...
    head_match.emplace("</html>");
    return head_match->ok();
  }

  std::optional<re2::RE2> head_match;
};
...
class MyHttpContext : public Context {
 public:
  ...
  FilterDataStatus onRequestBody(size_t body_length,
    ...
    std::string temp = recaptcha_added ? "true" : "false";
    LOG_INFO(temp);
    // if (recaptcha_added) {
    ...
    recaptcha_added = re2::RE2::Replace(&body_string, *root_->head_match, "<script src=\"https://www.google.com/recaptcha/enterprise.js?render=KEY_ID\"></script></head>");
    if (recaptcha_added) {
  ...
  FilterDataStatus onResponseBody(size_t body_length,
    ...
    std::string body_string = body->toString();
    re2::RE2::GlobalReplace(&body_string, *root_->head_match, "bar");
